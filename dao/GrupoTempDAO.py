class GrupoTempDAO:
    ...
    def usuario_esta_no_grupo(self, id_grupo, id_conta):
        return any(m.id_grupo == id_grupo and m.id_conta == id_conta for m in self.membros)

    def eh_lider(self, id_grupo, id_conta):
        """Líder = primeiro membro registrado no grupo."""
        membros = self.listar_membros_do_grupo(id_grupo)
        return bool(membros) and membros[0].id_conta == id_conta

    def excluir_grupo(self, id_grupo):
        """Remove o grupo e todos os seus membros da memória."""
        if id_grupo not in self.grupos_criados:
            return False
        self.membros = [m for m in self.membros if m.id_grupo != id_grupo]
        self.grupos_criados = [g for g in self.grupos_criados if g != id_grupo]
        return True

    def remover_membro(self, id_grupo, id_conta, id_personagem):
        """Remove um membro específico do grupo."""
        if id_grupo not in self.grupos_criados:
            return False

        antes = len(self.membros)
        self.membros = [
            m for m in self.membros
            if not (m.id_grupo == id_grupo and m.id_conta == id_conta and m.id_personagem == id_personagem)
        ]
        if len(self.membros) == antes:
            return False

        if not self.listar_membros_do_grupo(id_grupo):
            self.grupos_criados = [g for g in self.grupos_criados if g != id_grupo]
        return True

    def atualizar_membro(self, id_grupo, id_conta, id_personagem_novo, dao_perso):
        """Troca o personagem de um membro no grupo, respeitando as regras 1/1/3."""
        if id_grupo not in self.grupos_criados:
            return False

        idx = None
        for i, m in enumerate(self.membros):
            if m.id_grupo == id_grupo and m.id_conta == id_conta:
                idx = i
                break
        if idx is None:
            return False

        antigo = self.membros.pop(idx)
        if antigo.id_personagem == id_personagem_novo:
            self.membros.insert(idx, antigo)
            return True

        if self.adicionar_ao_grupo(id_grupo, id_conta, id_personagem_novo, dao_perso):
            return True
