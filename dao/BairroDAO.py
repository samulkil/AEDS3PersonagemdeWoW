...
class BairroDAO:
    ...
    def buscar_por_dono(self, id_dono):
        """Busca bairros pelo ID do dono usando Árvore B+ e retorna ordenado por id."""
        ...
        # Fallback: se o índice secundário não devolver resultados,
        # faz varredura completa para garantir consistência.
        if not bairros:
            with open(self.arquivo, "rb") as f:
                f.seek(self.header_size)
                while True:
        ...
        # Ordenação por ID do bairro (aproveita a propriedade de ordenação da Árvore B+)
        bairros.sort(key=lambda b: b.id)
        return bairros

    def update(self, id_alvo, novo_nome, novo_id_dono):
        """Atualiza um bairro."""
        # Localiza posição via Árvore B+ (índice primário).
        pos = self.arvore_id.buscar(id_alvo)

        # Fallback: varredura sequencial caso o índice esteja inconsistente.
        if pos is None:
            with open(self.arquivo, "rb") as f:
                f.seek(self.header_size)
        ...
        with open(self.arquivo, "rb+") as f:
            # Regrava o registro na mesma posição
            f.seek(pos)
        ...
        # Mantém índice secundário por dono atualizado para novas buscas.
        # Não removemos o dono antigo (não há remoção na árvore),
        # mas as leituras validam id_dono e lapide do registro.
        try:
